...
- Para usuários macOS/Linux/windows(vscode): o terminal já possui cliente SSH nativo.
...
*   **Par de chaves (login):** Esta é a etapa mais importante para a segurança.
    ...
    3.  Mantenha o **"Tipo de par de chaves"** como **RSA** e o **"Formato de arquivo de chave privada"** como **`.pem`** (para usuários de Mac/Linux/windows(VsCode)) ou **`.ppk`** (se você for usar o PuTTY no Windows).
...
**Passo 6:** Conecte-se à Instância no próprio console 
1.  Com a instância selecionada, clique no botão **"Connect"** no topo da página.
2. Escolha EC2 Instance Connect (browser-based SSH connection) e clique em **"Connect"**. Uma nova janela de terminal será aberta no navegador, conectando você à instância sem precisar de software adicional.
3. Clique em **"Connect"**.
4. Testar o usuário que você está, digitando o comando `whoami` no terminal. O retorno deve ser `ec2-user`, confirmando que você está logado na instância. Testar se o openssh-server está instalado, digitando o comando `sudo yum install openssh-server` no terminal. 

```bash

# teste se o openssh-server está instalado
sudo yum install openssh-server

# teste se o serviço está rodando
...
# verificar se a porta 22 está aberta e escutando
sudo netstat -tuln | grep :22

# verificar se o firewall está permitindo conexões na porta 22
sudo firewall-cmd --list-all

# caso o firewall esteja ativo, permitir a porta 22
...
#### **Cenário D:** Conectando via senha

Para conectar via senha, você precisará ter a senha da instância EC2. Isso é menos seguro e não é recomendado em ambientes de produção.

Recomenda-se sempre usar o método de chave pública/privada para autenticação SSH, pois é mais seguro e evita ataques de força bruta.

No config do SSH, você pode habilitar a autenticação por senha, mas isso deve ser feito com cautela e apenas em ambientes de teste.

# Como configurar o SSH para permitir autenticação por senha:
1.  Conecte-se à instância via console:
    - Acesse o AWS Management Console.
    - Vá para a seção "EC2".
    - Selecione a instância desejada.
    - Clique em "Conectar" e siga as instruções para obter a senha.
